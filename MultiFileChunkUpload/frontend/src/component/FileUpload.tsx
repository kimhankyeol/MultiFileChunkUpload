...
export default function FileUpload({fileItems, setFileItems} :{fileItems:Array<FileItem>, setFileItems: React.Dispatch<React.SetStateAction<FileItem[]>>}) {
  // 업로드할 파일들을 담을 State!
  ...
  /**
   * 파일업로드 버튼 클릭 Handler
   * 해당 method에서는 api를 호출해 file을 백엔드로 전송하고, 응답값에 대한 처리
   */
  const fileUploadHandler = async () => {
    ...
    if(file !== undefined) {
      try{
        ...
        const axiosResponse = await axiosInstance.post<ApiResponse<FileItem>>("/upload", formData, {"headers" : {"content-type" : "multipart/form-data"}})

        // HttpStatus가 200번호이
        // 서버에서 응답 코드로 0(성공)을 주지 않았을 경우
        // if(axiosResponse.status < 200 || axiosResponse.status >= 300 || axiosResponse.data.resultCode !== 0){
        //   // Error를 발생시켜 Catch문을 타게 만들어주는데, 서버에 응답받은 메시지
        //   // 서버에서 응답 메시지를 받지 못했을경우 기본 메시지 설정또한 함께 해준다

        //   console.table(axiosResponse)
        //   throw Error(axiosResponse.data.mes ?? "문제가 발생했어요!");
        // }
        // 파일 업로드 성공!
        alert('파일 업로드 성공!')
        const res:FileItem = axiosResponse.data.resultData;
  ...
  const fileExtensionValid = ({name} : {name : string}):boolean =>{
    ...
    /**
     * 허용가능한 확장자가 있는지 확인하는 부분은 indexOf를 사용해도 괜찮음 
     * 새롭게 나온 includes를 사용해도 괜찮고, 그밖의 다른 방법을 사용
  ...
  const removeFileName = (originalFileName:string):string => {
    ...
    // substring을 함수를 이용해 확장자만 잘라줌
    // lastIndex의 값은 마지막 .의 위치이기 때문에 해당 위치 다음부터 끝까지 문자열을 잘라줌
    // 문자열을 자른 후 소문자로 변경시켜 확장자 값을 반환 해줌
